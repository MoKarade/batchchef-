...
# Fragment keywords that identify produce items by canonical_name.
# Conservative: err toward grocery (non-produce) when uncertain.
_PRODUCE_NAME_FRAGMENTS = frozenset({
    # Légumes
    "tomate", "carotte", "courgette", "aubergine", "poivron", "concombre",
    "laitue", "salade", "epinard", "brocoli", "chou", "poireau", "oignon",
    "echalote", "champignon", "asperge", "artichaut", "betterave", "navet",
    "celeri", "panais", "radis", "fenouil", "endive", "cresson", "roquette",
    "mais", "pois", "haricot", "feve", "edamame", "courge", "citrouille",
    "potiron", "patisson", "salsifis", "rutabaga", "topinambour",
    # Fruits
    "pomme", "poire", "peche", "prune", "cerise", "fraise", "framboise",
    "mure", "myrtille", "bleuet", "raisin", "melon", "pasteque", "ananas",
    "mangue", "kiwi", "banane", "orange", "citron", "lime", "pamplemousse",
    "clementine", "mandarine", "figue", "abricot", "datte", "avocat",
    "grenade", "papaye", "goyave", "litchi", "longan",
    # Ail
    "ail",
})


def _is_produce_heuristic(canonical_name: str) -> bool:
    """True if canonical_name contains a known produce fragment."""
    parts = frozenset(canonical_name.lower().split("_"))
    return bool(parts & _PRODUCE_NAME_FRAGMENTS)
...
async def _run(job_id: int, urls: list[str]):
    ...
    async with async_playwright() as pw:
        ...
        if not cancelled and new_ingredient_ids:
            # L2: classify new ingredients as produce vs grocery (heuristic first,
            # exact AI classification happens in the L8 full-backfill).
            produce_ids, grocery_ids = await _split_produce_grocery(sorted(new_ingredient_ids))

            await manager.broadcast(str(job_id), {
                "job_id": job_id,
                "status": "pricing",
                "message": (
                    f"Recherche prix Maxi/Costco pour {len(grocery_ids)} ingrédients "
                    f"({len(produce_ids)} légumes/fruits → Fruiterie)…"
                ),
            })

            # Generate search aliases for grocery items only (produce go to Fruiterie)
            if grocery_ids:
                await _generate_aliases_for_ingredients(grocery_ids)

            # Deduplicate all new ingredients
            await _deduplicate_ingredients(sorted(new_ingredient_ids))

            # Scrape Maxi + Costco for grocery items only
            priced_ids, unpriced_ids = await _price_new_ingredients(grocery_ids, pages)

            # Gate recipes: mark complete/incomplete based on grocery price coverage.
            # Produce ingredients are excluded from unpriced_ids — they'll be estimated
            # asynchronously by Fruiterie and should not block recipe completion.
            quarantine_lines = await _gate_recipes(all_new_recipe_ids, unpriced_ids)
            if quarantine_lines:
                errors.extend(quarantine_lines)

            logger.info(
                f"Import {job_id}: {len(produce_ids)} produce → Fruiterie, "
                f"{len(grocery_ids)} grocery → Maxi/Costco "
                f"({len(priced_ids)} priced, {len(unpriced_ids)} missed)"
            )
    ...
    # Auto-cascade: Fruiterie estimation for produce items (async bonus, non-blocking)
    if not cancelled and new_ingredient_ids:
        produce_ids_final, _ = await _split_produce_grocery(sorted(new_ingredient_ids))
        if produce_ids_final:
            await _dispatch_fruiterie_job(job_id, produce_ids_final)


async def _split_produce_grocery(ingredient_ids: list[int]) -> tuple[list[int], list[int]]:
    """Split IDs into (produce_ids, grocery_ids) using heuristic + DB is_produce flag.

    Priority: DB flag wins if already set. Otherwise apply _is_produce_heuristic()
    and persist the result so future calls are consistent.
    """
    from sqlalchemy import select
    ...
    async with AsyncSessionLocal() as db:
        ...
        updated = False
        for ing in ings:
            # Apply heuristic if not yet classified
            if not ing.is_produce and _is_produce_heuristic(ing.canonical_name):
                ing.is_produce = True
                updated = True
            if ing.is_produce:
                produce_ids.append(ing.id)
            else:
                grocery_ids.append(ing.id)

        if updated:
            await db.commit()

    return produce_ids, grocery_ids
...
async def _price_new_ingredients(
    ...
    """Scrape Maxi + Costco for each new ingredient using multi-query + AI validation.
    ...
    async with AsyncSessionLocal() as db:
        stores_q = select(Store).where(Store.code.in_(["maxi", "costco"]))
...
async def _dispatch_fruiterie_job(parent_job_id: int, ingredient_ids: list[int]):
    """Queue Fruiterie price estimation as async bonus job."""
    from app.database import AsyncSessionLocal
    from app.models.job import ImportJob
    ...
    logger.info(
        f"Auto-dispatched fruiterie estimate after import {parent_job_id}: "
        f"job={fruit_id}, ingredients={len(ingredient_ids)}"
    )
