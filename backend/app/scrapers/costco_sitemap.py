...
def search(query: str, max_results: int = 20) -> list[tuple[str, str, int]]:
    """Fuzzy search the catalogue.

    Returns [(itemId, slug, score)*] — higher score = better match.
    Score = number of query tokens that appear in the slug.
    """
    ...
    hits: dict[str, int] = {}
    for tok in tokens:
        for item_id in _token_index.get(tok, ()):
            hits[item_id] = hits.get(item_id, 0) + 1
    # Also prefix-match for short tokens (eg. "oeuf" → matches "oeufs")
    for tok in tokens:
        if len(tok) < 6:
            for indexed_tok, ids in _token_index.items():
                if indexed_tok.startswith(tok) and indexed_tok != tok:
                    for item_id in ids:
                        hits[item_id] = hits.get(item_id, 0) + 0.5  # partial credit
    scored = sorted(hits.items(), key=lambda kv: -kv[1])[:max_results]
    return [(iid, _catalogue.get(iid, ""), int(score)) for iid, score in scored]
