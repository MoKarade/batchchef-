...
# Kill by port — catches uvicorn (:8001), Next.js (:3000), and the legacy
# :8000 if it's still in use, even if parent PowerShell windows closed uncleanly.
...
# Kill node.exe spawned from this repo's frontend (matches both the
# current "batchchef" path and the legacy "batch-cooking" location).
Get-CimInstance Win32_Process -Filter "Name = 'node.exe'" -ErrorAction SilentlyContinue |
    Where-Object { $_.CommandLine -match '(batchchef|batch-cooking)[\\/]frontend' } |
    ForEach-Object {
